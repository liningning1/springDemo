...
public class ResourcesController {
	...
	public String deleteById(Model model,String resourcesId){
		resourcesService.delete(Integer.parseInt(resourcesId));
		return "redirect:query.html";
	}
	...
	public String getById(Model model,String resourcesId,int typeKey){
		Resources resources = resourcesService.getById(Integer.parseInt(resourcesId));
		model.addAttribute("resources", resources);
	...
	public String permissioUser(Model model,String userId){
		List<Resources> resources = resourcesService.getUserResources(Integer.parseInt(userId));
	...
	public String permissioRole(Model model,String roleId){
		List<Resources> resources = resourcesService.getRoleResources(Integer.parseInt(roleId));
		List<Resources> allRes = resourcesService.findAll();
		StringBuffer sb = new StringBuffer();
		sb.append("var data = [];");
		for (Resources r : allRes) {
	...
	public String saveRoleRescours(String roleId,String rescId){
		...
		try {
			//resourcesService.saveRoleResources(Integer.parseInt(roleId), list);
		} catch (Exception e) {
			e.printStackTrace();
			errorCode="1001";
		}
